<?php
    ...
    $result_edit = $userdata1->rq_edit($id);
    ...
    if (isset($_POST['submit1'])){
        ...
        $sql1 = $userdata1->rq_update($id,$objective_rq,$num_passengers,$date_go_rq,$date_back_rq,$pho_rq, $email_rq);

        if($sql1) {
            echo "<script>alert('แก้ไขคำขอเรียบร้อย')</script>";
...
<body>
    ...
    <div class="container">
                <h1 class="mt-5"> กรุณากรอกลายละเอียดเพื่อแก้ไข</h1>
                <hr>
                <form method="post">
                    <div class="mb-3">
                        <label for="objective" class="form-label">จุดประสงค์ในการขอใช้ยานพาหนะ : </label>
                        <input type="text" class="form-control" id="objective_rq" name="objective_rq" value="<?php echo $row["objective_rq"];?>" required>
                    </div>
                    <div class="mb-3">
                        <label for="num_passengers" class="form-label">จำนวนผู้โดยสาร :</label>
                        <input type="text" class="form-control" id="num_passengers" name="num_passengers" value="<?php echo $row["num_passengers"];?>" required>
                    </div>
                    <div class="mb-3">
                        <label for="date_go_rq" class="form-label">วันที่เดินทางไป :</label>
                        <input type="date" id="date_go_rq" name="date_go_rq" value="<?php echo $row["date_go_rq"];?>" required>
                    </div>
                    <div class="mb-3">
                        <label for="date_back_rq" class="form-label">วันที่เดินทางกลับ :</label>
                        <input type="date" id="date_back_rq" name="date_back_rq" value="<?php echo $row["date_back_rq"];?>" required>
                    </div>
                    <div class="mb-3">
                        <label for="pho_rq" class="form-label">เบอร์ติดต่อ :</label>
                        <input type="text" class="form-control" id="pho_rq" name="pho_rq" value="<?php echo $row["pho_rq"];?>" required>
                    </div>
                    <div class="mb-3">
                        <label for="email_rq" class="form-label">Email สำหรับติดต่อ :</label>
                        <input type="text" class="form-control" id="email_rq" name="email_rq" value="<?php echo $row["email_rq"];?>" required>
                    </div>
                   
                    <button type="submit" name ="submit1" id="submit1" class="btn btn-primary">แก้ไขคำขอ</button>
                </form>
